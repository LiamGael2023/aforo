// Dashboard JavaScript para Sistema de Aforo Digital - PHP MVC
            ...
            <div class="station-item ${selectedStationId == station.id ? 'selected' : ''}"
                ...
                <div class="station-details">
                    <span class="station-level">
                        Nivel: ${station.latest_level ? parseFloat(station.latest_level).toFixed(2) + 'm' : 'N/A'}
                    </span>
                    <span class="station-flow">
                        Caudal: ${station.latest_flow ? parseFloat(station.latest_flow).toFixed(3) + ' m³/s' : 'N/A'}
                    </span>
...
function renderChart(data) {
    ...
    mainChart = new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                title: { display: true, text: `${data.station_name} (${data.station_code})` },
                legend: { position: 'top' }
            },
            scales: {
                x: {
    ...
    try {
        ...
        tbody.innerHTML = measurements.map(m => `
            <tr>
                ...
                <td>${parseFloat(m.water_level).toFixed(2)}</td>
                <td>${m.flow_rate ? parseFloat(m.flow_rate).toFixed(3) : 'N/A'}</td>
                <td>
...
async function createStation(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert('Error: ' + (error.error || 'No se pudo crear la estación'));
        }
    } catch (error) {
...
async function createMeasurement(event) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const error = await response.json();
            alert('Error: ' + (error.error || 'No se pudo registrar la medición'));
        }
    } catch (error) {
